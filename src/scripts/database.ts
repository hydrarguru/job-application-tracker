...
const DATABASE_NAME: string = 'ApplicationDatabase';
const DATABASE_TABLE: string = 'ApplicationTable';
...
export function createDatabase(databaseName?: string) {
    if (!databaseName) {
        ...
        return openDB<JobDB>(databaseName, 1, {
            upgrade(db) {
                const store = db.createObjectStore('ApplicationTable', {
                    keyPath: 'id',
                    autoIncrement: true,
                });
...
export async function updateDbData(data: IAppliedJob, id: number) {
    const db = await openDB(DATABASE_NAME);
    const tx = db.transaction(`${DATABASE_TABLE}`, 'readwrite');
